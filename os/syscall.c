...
static void handle_request_secure_storage_creation_syscall(uint8_t runtime_proc_id,
							   uint8_t *buf)
{
...
static void handle_request_secure_storage_access_syscall(uint8_t runtime_proc_id,
							 uint8_t *buf)
{
	SYSCALL_GET_ONE_ARG
	uint32_t count = arg0;
...
static void handle_delete_secure_storage_syscall(uint8_t runtime_proc_id,
						 uint8_t *buf)
{
	struct runtime_proc *runtime_proc = get_runtime_proc(runtime_proc_id);
	if (!runtime_proc || !runtime_proc->app) {
