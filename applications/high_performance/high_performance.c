...
void *senderThread(struct runtime_api *api)
{
	char line[1024];
	int size;
...
void *receiveThread(struct runtime_api *api)
{
	char line[1024];
	int size;
	uint8_t target_qid = 0;
...
void app_main(struct runtime_api *api)
{
	...
	pthread_create(&sender_thread, NULL, sender_thread, api);
	phtread_create(&receiver_thread, NULL, receiver_thread, api);
