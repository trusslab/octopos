...
void runtime_send_msg_on_queue(uint8_t *buf, uint8_t queue_id)
{
     ...
     */
    MBOX_PENDING_STA = Mbox_regs[queue_id];
    ...
    if (mask & XMB_IX_STA) {
        if (callback_ref == &Mbox_out) {
            ...
            if (MBOX_PENDING_STA == &Mbox_out) {
                ...
                MBOX_PENDING_STA = NULL;
            }
        } else if (callback_ref == &Mbox_sys) {
        	/* Syscall request */
            if (MBOX_PENDING_STA == &Mbox_sys) {
